# PenRenderer Spike（已弃用的历史原型）

> **状态：DEPRECATED。** 当前唯一完整、正式且视觉效果更好的 Pen/Pencil 实现位于 [`../../psd2pen/`](../../psd2pen/)。本目录仅保留早期技术探索和可复现实验，不得作为生产渲染器、视觉基准、新功能入口或 `.pen` 修改方案。后续 Pen 开发只能进入 `psd2pen/`，并遵守其中的 `AGENTS.md` 与 Pencil MCP 工作流。
